-- PokeAPI SQLite Schema
-- This schema mirrors the PokeAPI data structure for offline querying
-- Comments explain which endpoint populates each table
...
-- Query 1: "What Pokemon can I catch in Pokemon Black?"
-- 
-- SELECT s.name, pe.entry_number
-- FROM species s
-- JOIN pokedex_entries pe ON s.id = pe.species_id
-- JOIN pokedexes p ON pe.pokedex_id = p.id
-- JOIN version_group_pokedexes vgp ON p.id = vgp.pokedex_id
-- JOIN version_groups vg ON vgp.version_group_id = vg.id
-- JOIN versions v ON v.version_group_id = vg.id
-- WHERE v.name = 'black'
-- ORDER BY pe.entry_number;

-- Query 2: "What are Pikachu's stats?"
--
-- SELECT hp, attack, defense, special_attack, special_defense, speed
-- FROM pokemon
-- WHERE name = 'pikachu' AND is_default = TRUE;

-- Query 3: "What moves can Pikachu learn in Pokemon Black, and how?"
--
-- SELECT m.name, m.type_name, m.power, pm.level_learned_at, pm.learn_method
-- FROM pokemon_moves pm
-- JOIN moves m ON pm.move_id = m.id
-- JOIN pokemon p ON pm.pokemon_id = p.id
-- JOIN version_groups vg ON pm.version_group_id = vg.id
-- JOIN versions v ON v.version_group_id = vg.id
-- WHERE p.name = 'pikachu' AND v.name = 'black'
-- ORDER BY 
...
-- Query 4: "How does Eevee evolve?"
--
-- SELECT 
--     fs.name as from_pokemon,
--     ts.name as to_pokemon,
--     e.trigger_name,
--     e.min_level,
--     e.item_name,
--     e.min_happiness,
...
-- Query 5: "What are the different forms of Meowth?"
--
-- SELECT p.name, p.is_default, 
...
-- Query 6: "Show me all Fire-type Pokemon available in Pokemon Sword"
--
-- SELECT DISTINCT s.name, pe.entry_number
-- FROM species s
-- JOIN pokemon p ON s.id = p.species_id AND p.is_default = TRUE
-- JOIN pokemon_types pt ON p.id = pt.pokemon_id
-- JOIN pokedex_entries pe ON s.id = pe.species_id
-- JOIN pokedexes pd ON pe.pokedex_id = pd.id
-- JOIN version_group_pokedexes vgp ON pd.id = vgp.pokedex_id
-- JOIN version_groups vg ON vgp.version_group_id = vg.id
-- JOIN versions v ON v.version_group_id = vg.id
-- WHERE v.name = 'sword' AND pt.type_name = 'fire'
-- ORDER BY pe.entry_number;
